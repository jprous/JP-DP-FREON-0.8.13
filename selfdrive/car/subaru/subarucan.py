...
def create_es_lkas_fwd(packer, es_lkas_fwd_msg, frame, steer_step):
  values = copy.copy(es_lkas_fwd_msg)
...
def create_es_lkas(lkas_mode, sng_acc_resume, packer, es_lkas_msg, enabled, visual_alert, left_line, right_line, left_lane_depart, right_lane_depart, cruis_lkas_btn, mchange):
  ...
  if (lkas_mode == 3):
    ...
    values["LKAS_Dash_State"] = 1 # LKAS White
    
    if enabled:
  
      #values["LKAS_Enable_1"] = 1  # Try to get LKAS to stop
      values["LKAS_Dash_State"] = 2 # LKAS Green
      #values["Signal5"] = 0    #JP Test Set to Wilderness
      values["LKAS_Left_Line_Visible"] = 2     #Green
      values["LKAS_Right_Line_Visible"] = 2  #Green
    else:
      values["LKAS_Left_Line_Visible"] = 1     #White
      values["LKAS_Right_Line_Visible"] = 1  #White
  
  elif (lkas_mode == 2):
    
    #if es_lkas_steer == 0:
    #  values["LKAS_Dash_State"] = 1 # LKAS Standby
    #else:
    #  values["LKAS_Dash_State"] = 2 # LKAS Steering
...
def create_es_dashstatus(packer, dashstatus_msg, lkas_mode):
