...
class CarControllerParams:
  def __init__(self, CP):
    if CP.carFingerprint == CAR.IMPREZA_2020:
      ...
      self.STEER_MAX = 2047            # 2047 self steer max                                #JP best set 3071
    self.STEER_STEP = 2                # 2 how often we update the steer cmd
    self.STEER_DELTA_UP = 50           # 50 torque increase per refresh, 0.8s to max        #JP best set 150
    self.STEER_DELTA_DOWN = 70         # 70 torque decrease per refresh                     #JP best set 170
    self.STEER_DRIVER_ALLOWANCE = 60   # 60 allowed driver torque before start limiting     #JP best set 160
