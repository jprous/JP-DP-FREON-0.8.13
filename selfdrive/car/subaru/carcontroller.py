...
class CarController:
  def __init__(self, dbc_name, CP, VM):
    ...
    self.speed_limit = 0
    self.turn_limit = 0
    self.spdlim_dist = 1000
    self.prev_spdlim_dist = 1000
    self.spdlim_dist_down = True
    self.map_lim = False
    self.prev_speed_limit = 16.666
    ...
    self.speed_limit_value_offset = 5 # int(self.param_s.get("SpeedLimitValueOffset"))
  ...
  def update(self, CC, CS):
    ...
    if self.CP.carFingerprint in PREGLOBAL_CARS:
      ...
      #check every half second for speed change
      if ((self.frame % 20) == 0):
        ...
        if self.speed_limit_control_enabled:
          ...
          self.get_spdlim_dist()
          self.get_ismap_limit()
          
          if self.prev_speed_limit>9:
            self.prev_rd_spd = self.prev_speed_limit
          
          #Check if distance is increasing or decreasing
          self.spdlim_dist_down = False
          if (abs(self.spdlim_dist) < abs(self.prev_spdlim_dist)) and (self.spdlim_dist != 1000):
            self.spdlim_dist_down = True
            self.cs_const_ind = 5000
          elif (abs(self.spdlim_dist) > abs(self.prev_spdlim_dist)) and (self.spdlim_dist != 1000):
            self.cs_const_ind = 1000
          else:
            self.cs_const_ind = 0
          
          
          #CS.speed_limit = self.spdlim_dist
        #else:
        #  CS.speed_limit = 0
        ...
        #check to see if the speed limit has changed in m/s (35kph +- = 9m/s) (72kph +- = 20m/s)
        if (self.speed_limit != self.prev_speed_limit) and (self.speed_limit_control_enabled):
          self.speed_lim_change = True

          if abs(self.speed_limit - self.prev_speed_limit) > 20:                                            #keep the same if change is more than 80 kph or 20m/s
            self.speed_limit = self.prev_rd_spd
            self.speed_lim_change = True
          elif (self.speed_limit < 9) and (self.speed_limit != 0) and ((self.spdlim_dist == 1000) or (self.spdlim_dist < 10) or (self.spdlim_dist_down == False) or (CS.out.vEgo < 10)):     #set the speed limit to the previous value when past the node feature
            self.speed_limit = self.prev_rd_spd
            self.speed_lim_change = True
          #elif (self.speed_limit < 9) and (self.spdlim_dist < 100):                                         #set the speed limit to the node limit when closer than 70m and save the road speed limit
            #self.speed_lim_change = True
            #self.prev_rd_spd = max(self.prev_rd_spd, self.prev_speed_limit)
          elif (self.speed_limit < 9) and (self.speed_limit != 0) and (self.spdlim_dist > CS.out.vEgo * 8) and (self.spdlim_dist < 1000):        #dont change the speed limit yet, wait for below m/s speed * 8
            self.speed_limit = self.prev_rd_spd
            self.speed_lim_change = True #  self.speed_limit = self.prev_speed_limit
          
           # self.speed_lim_change = True                                                       #if none of the above, change the speed limit and update the last road speed limit
        
        #Reset to the previous road limit if cruise speed is below 20 km/h (5m/s)
        if (CS.out.vEgo < 5):
          self.speed_limit = self.prev_rd_spd
          self.speed_lim_change = True
      ...
      if (self.speed_lim_change) and (self.speed_limit != 0) and (((self.frame + 10) % 20) == 0):
        ...
        if min(new_speedlim, new_tunrlim) > 0:
          CS.speed_limit = min(new_speedlim, new_tunrlim) + self.cs_const_ind    ### JP TEST
        ...
        #if speed limit is defined and less than lower limit, and turn limit is less than set speed limit
        if ((new_speedlim < self.splim_min) and (new_speedlim > 0)):
          new_cspeed = 25
          self.adjust_speed = True
        ...
        if ((CS.cruise_state == 0) and (CS.out.cruiseState.enabled) and (CS.speed_limit!=0)):
          ...
          new_cspeed = new_cspeed + self.speed_limit_value_offset
          
          #if self.speed_limit_perc_offset == 0:           
          #  new_cspeed = new_cspeed + self.speed_limit_value_offset
          #else:
          #  new_cspeed = new_cspeed * self.speed_limit_perc_offset / 100
      ...
      if ((self.frame % 20) == 0):
        if self.speed_limit_control_enabled:
          ...
          self.prev_spdlim_dist = self.spdlim_dist
  ...
  def get_ismap_limit(self):
    self.map_lim = self.sm['longitudinalPlan'].isMapSpeedLimit 
    return self.map_lim
  ...
  def get_spdlim_dist(self):
    self.spdlim_dist = float(self.sm['longitudinalPlan'].distToSpeedLimit if self.sm['longitudinalPlan'].distToSpeedLimit is not None else 1000) 
    if self.spdlim_dist == 0:
      self.spdlim_dist = 1000
    return self.spdlim_dist
